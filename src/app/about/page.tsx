        ...
        <div className="container mx-auto px-4">
          <div className="grid lg:grid-cols-2 gap-24 items-center">
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-6 pt-6">
                {[
                  ...
                  <div key={i} className="p-8 rounded-[32px] bg-slate-50 border border-slate-100 group hover:bg-white hover:shadow-xl transition-all">
                    <p className="text-4xl font-bold text-brand-navy mb-2">{stat.value}<span className="text-brand-gold text-2xl ml-1">{stat.unit}</span></p>
            ...
            <motion.div 
              ...
              className="relative"
            >
              <div className="relative z-10 rounded-[60px] overflow-hidden shadow-2xl border-[12px] border-slate-50">
                <Image src="/images/about-factory.jpg" alt="MRC Üretim" width={800} height={1000} className="object-cover scale-105" />
              </div>
              <div className="absolute -top-10 -right-10 w-40 h-40 bg-brand-gold rounded-[40px] -z-10 rotate-12" />
              <div className="absolute -bottom-10 -left-10 w-40 h-40 bg-brand-navy rounded-[40px] -z-10 -rotate-6" />
            </motion.div>
          </div>
        </div>
      </section>
      ...
      <section className="py-32 bg-[#050B15] relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="text-center max-w-3xl mx-auto mb-20">
            <p className="text-brand-gold font-bold uppercase tracking-widest text-sm mb-4">Vizyon & Misyon</p>
            <h2 className="text-4xl lg:text-5xl font-bold text-white font-display">Geleceğin Yapılarını Bugün İnşa Ediyoruz</h2>
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-12">
            {[
              ...
              <motion.div 
                key={idx}
                whileHover={{ y: -10 }}
                className="p-12 bg-white/5 rounded-[48px] border border-white/10 hover:border-brand-gold/30 hover:bg-white/[0.07] transition-all group"
              >
                <div className="w-20 h-20 rounded-[28px] bg-brand-gold text-brand-navy flex items-center justify-center mb-10 shadow-lg shadow-brand-gold/20 group-hover:scale-110 transition-transform">
                  {value.icon}
                </div>
                <h3 className="text-3xl font-bold text-white mb-6 font-display">{value.title}</h3>
                <p className="text-slate-400 text-lg leading-relaxed font-light">{value.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="py-32 bg-white">
        <div className="container mx-auto px-4">
          <div className="text-center mb-20">
            <h2 className="text-4xl font-bold text-brand-navy font-display mb-4">Neden MRC Yalıtım?</h2>
      ...
      <section className="py-20 bg-brand-gold">
        <div className="container mx-auto px-4 flex flex-col lg:flex-row justify-between items-center gap-12">
          <div className="text-brand-navy space-y-3">
            <h2 className="text-3xl lg:text-5xl font-black font-display">Tecrübemize Güvenin.</h2>
            <p className="text-brand-navy/70 text-lg lg:text-xl font-medium">Büyük projeleriniz için özel üretim ve teknik danışmanlık sağlıyoruz.</p>
          </div>
          <div className="flex gap-4">
            <button className="bg-brand-navy text-white px-10 py-5 rounded-2xl font-bold text-lg hover:scale-105 transition-transform shadow-2xl">
              Projeleri İncele
            </button>
          </div>
        </div>
