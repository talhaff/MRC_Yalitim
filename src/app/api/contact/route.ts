...
// Cloudflare Pages Edge Runtime bildirimi
...
export async function POST(request: Request) {
  try {
    ...
    const recipientEmail = 'mrcyalitim@gmail.com';
    const emailSubject = `[MRC Web İletişim Formu] ${subject} - ${name}`;
    ...
    // 1. Edge-Native E-Posta Gönderimi: Resend API (Önerilen & Standart)
    ...
    if (resendApiKey) {
      const resendResponse = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          from: process.env.EMAIL_FROM || 'MRC Web Sitesi <onboarding@resend.dev>',
          to: [recipientEmail],
          reply_to: email,
    ...
    // 2. Edge-Native E-Posta Gönderimi: MailChannels (Cloudflare Ücretsiz Relay)
    else if (process.env.ENABLE_MAILCHANNELS === 'true') {
      try {
        await fetch('https://api.mailchannels.net/tx/v1/send', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            personalizations: [{ to: [{ email: recipientEmail, name: 'MRC Yalıtım Söve' }] }],
            from: { email: 'noreply@mrcsoveyalitim.com', name: `${name} (MRC Web)` },
        ...
        console.error('MailChannels error:', mcErr);
      }
    } else {
      // Geliştirme ve önizleme ortamında güvenli log kaydı
      console.log('--- [EDGE RUNTIME] YENİ İLETİŞİM FORMU MESAJI ---');
